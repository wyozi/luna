local __L_as,__L_to,__L_gmt=assert,type,getmetatable;local function __L_t(o)local t=__L_to(o) if t=="table" then local mt = __L_gmt(o)return (mt and mt.__type) or t end return t end;local Lexer = {  }
Lexer.__index = Lexer
...
function Lexer:_readBinLiteral()
	local __ifa0_n = self:_readToken("number", "^0b[10_]+"); if __ifa0_n then local n = __ifa0_n
	local bin = n.text:gsub("[^01]", "")

	local __ifa1_dec = tonumber(bin, 2); if __ifa1_dec then local dec = __ifa1_dec
